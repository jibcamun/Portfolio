...
The reward is computed as a piecewise function:

- Valid `segment`: `sum((20 / N) * (2I[o] - 1))` for each object `o` in `O`
- Valid `adjust`: `clamp((30 / N) * (score(o, p_new) - score(o, p_old)), -30 / N, 30 / N)`
- Valid `place`: `clamp((50 / N) * (L(P_new) - L(P_old)), -50, 50)`
- Invalid `segment`: `-20 / N`
- Invalid `adjust`: `-30 / N`
- Invalid `place`: `-50 / N`
