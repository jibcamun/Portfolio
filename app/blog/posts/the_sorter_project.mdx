...
The reward is computed from the quality of the agent's action:

- A valid `segment` action receives partial credit for each correctly identified object and loses credit for incorrect identifications.
- A valid `adjust` action is rewarded when moving an object improves its local placement score, and penalized when the move makes the placement worse.
- A valid `place` action is rewarded when the proposed full layout improves the total layout score across all objects.
- Invalid `segment`, `adjust`, and `place` actions receive task-specific penalties.
